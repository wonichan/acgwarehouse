...
ACGWarehouse 已于 `v3.0` 完成“导入后任务平台化”里程碑，当前产品已经具备图片扫描入库、AI 自动标签、重复检测、搜索、收藏夹、多端访问，以及导入后自动入队、批次监控、队列控制、补跑恢复与失败诊断能力。项目现已进入 `v4.0` 规划阶段，目标转向基于 Windows Photos 体验范式的桌面重构，并把图像计算链路从现有 Go 主控中拆出受控的 Python 计算侧车。
...
| **v4.0: Python 仅承担计算层职责** | 保持 Go 继续负责编排、任务平台、持久化与进程治理，避免职责混乱 | — Pending |
