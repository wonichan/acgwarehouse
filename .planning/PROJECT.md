...
ACGWarehouse 已于 `v3.0` 完成“导入后任务平台化”里程碑，当前产品已经具备图片扫描入库、AI 自动标签、重复检测、搜索、收藏夹、多端访问，以及导入后自动入队、批次监控、队列控制、补跑恢复与失败诊断能力。项目当前处于 `v4.0` 执行期，`Phase 15` 已完成计算侧车基础设施（Go 侧生命周期编排、运行时地址 manifest 发现、降级可用与可观测性边界），`Phase 16` 已完成重复检测计算迁移（Python 侧车计算、Go 编排落库、结构化推荐依据透传），下一步进入桌面主壳层重构阶段。
...
---
*Last updated: 2026-04-04 after Phase 16 completion*
