...
ACGWarehouse 已于 `v3.0` 完成“导入后任务平台化”里程碑，当前产品已经具备图片扫描入库、AI 自动标签、重复检测、搜索、收藏夹、多端访问，以及导入后自动入队、批次监控、队列控制、补跑恢复与失败诊断能力。项目当前处于 `v4.0` 执行期，`Phase 15` 已完成计算侧车基础设施（Go 侧生命周期编排、运行时地址 manifest 发现、降级可用与可观测性边界），`Phase 16` 已完成重复检测计算迁移（Python 侧车计算、Go 编排落库、结构化推荐依据透传），`Phase 17` 已完成桌面主壳层基础（顶栏搜索/导入/设置、方块网格图库工作区、右侧持久筛选面板），`Phase 18` 已完成独立查看器与 filmstrip，`Phase 19` 已完成标签治理工作区（治理列表、显式合并、安全删除、批量治理与图库联动），`Phase 20` 已完成运营监控增强（桌面监控入口、批次/任务下钻、sidecar 诊断与重启）。下一步进入 `Phase 21` Windows 打包。
...
- [ ] Windows Photos 风格桌面体验继续向运营可观测与治理效率收敛
