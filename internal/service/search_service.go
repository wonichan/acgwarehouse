...
func (s *SearchService) Search(ctx context.Context, opts SearchOptions) (*SearchResult, error) {
	...
	logger.Infof("[service] Search started: query_len=%d tag_count=%d sort_by=%s limit=%d", len(opts.Query), len(opts.TagIDs), opts.SortBy, opts.Limit)
	...
	case opts.Query != "" && len(opts.TagIDs) > 0:
		...
		if err != nil {
			logger.Errorf("[service] Search failed: operation=combinedSearch error=%v", err)
		}
	case opts.Query != "":
		images, total, err = s.ftsSearch(ctx, opts)
		if err != nil {
			logger.Errorf("[service] Search failed: operation=ftsSearch error=%v", err)
		}
	case len(opts.TagIDs) > 0:
		// Tag search only (use existing repository method)
		images, total, err = s.tagSearch(ctx, opts)
		if err != nil {
			logger.Errorf("[service] Search failed: operation=tagSearch error=%v", err)
		}
	default:
		// No filters: return all images
		images, total, err = s.allImages(ctx, opts)
		if err != nil {
			logger.Errorf("[service] Search failed: operation=allImages error=%v", err)
		}
	}
	...
	logger.Infof("[service] Search completed: result_count=%d total=%d has_more=%t", len(result.Images), result.Total, result.HasMore)
...
func (s *SearchService) ftsSearch(ctx context.Context, opts SearchOptions) ([]domain.Image, int64, error) {
	...
	if err != nil {
		logger.Errorf("[service] ftsSearch failed: operation=searchRepo.SearchImages error=%v", err)
		return nil, 0, err
	}
	...
	if err != nil {
		logger.Errorf("[service] ftsSearch failed: operation=searchRepo.CountSearchImages error=%v", err)
		return nil, 0, err
	}
...
func (s *SearchService) tagSearch(ctx context.Context, opts SearchOptions) ([]domain.Image, int64, error) {
	...
	if err != nil {
		logger.Errorf("[service] tagSearch failed: operation=imageRepo.FindByTagIDs error=%v", err)
		return nil, 0, err
	}

	total, err := s.imageRepo.CountByTagIDs(ctx, opts.TagIDs)
	if err != nil {
		logger.Errorf("[service] tagSearch failed: operation=imageRepo.CountByTagIDs error=%v", err)
		return nil, 0, err
	}
...
func (s *SearchService) combinedSearch(ctx context.Context, opts SearchOptions) ([]domain.Image, int64, error) {
	...
	if err != nil {
		logger.Errorf("[service] combinedSearch failed: operation=searchRepo.SearchImages error=%v", err)
		return nil, 0, err
	}
	...
	if err != nil {
		logger.Errorf("[service] combinedSearch failed: operation=searchRepo.CountSearchImages error=%v", err)
		return nil, 0, err
	}
	return images, total, nil
}
...
func (s *SearchService) allImages(_ context.Context, opts SearchOptions) ([]domain.Image, int64, error) {
	...
	if err != nil {
		logger.Errorf("[service] allImages failed: operation=imageRepo.FindAll error=%v", err)
		return nil, 0, err
	}

	total, err := s.imageRepo.Count()
	if err != nil {
		logger.Errorf("[service] allImages failed: operation=imageRepo.Count error=%v", err)
		return nil, 0, err
	}
...
func (s *SearchService) SearchByFilename(ctx context.Context, pattern string, limit, offset int) (*SearchResult, error) {
	...
	logger.Infof("[service] SearchByFilename started: pattern=%s limit=%d", pattern, limit)

	images, err := s.searchRepo.SearchByFilenames(ctx, pattern, limit, offset)
	if err != nil {
		logger.Errorf("[service] SearchByFilename failed: operation=searchRepo.SearchByFilenames error=%v", err)
		return nil, err
	}

	total, err := s.searchRepo.CountByFilenames(ctx, pattern)
	if err != nil {
		logger.Errorf("[service] SearchByFilename failed: operation=searchRepo.CountByFilenames error=%v", err)
		return nil, err
	}
	...
	logger.Infof("[service] SearchByFilename completed: result_count=%d total=%d has_more=%t", len(result.Images), result.Total, result.HasMore)
...
func (s *SearchService) ViewerWindow(ctx context.Context, opts SearchOptions, selectedIndex, limit int) (*ViewerWindowResult, error) {
	...
	logger.Infof("[service] ViewerWindow started: selected_index=%d limit=%d", selectedIndex, limit)
	if selectedIndex < 0 {
	...
	if err != nil {
		logger.Errorf("[service] ViewerWindow failed: operation=viewerWindowTotal error=%v", err)
		return nil, err
	}
	...
	if err != nil {
		logger.Errorf("[service] ViewerWindow failed: operation=viewerWindowImages error=%v", err)
		return nil, err
	}
	...
	logger.Infof("[service] ViewerWindow completed: window_size=%d total=%d selected_index=%d", len(result.Images), result.Total, selectedIndex)
...
func (s *SearchService) viewerWindowTotal(ctx context.Context, opts SearchOptions) (int64, error) {
	if opts.Query != "" {
		...
		if err != nil {
			logger.Errorf("[service] viewerWindowTotal failed: operation=searchRepo.CountSearchImages error=%v", err)
		}
		return total, err
	}
	if len(opts.TagIDs) > 0 {
		total, err := s.imageRepo.CountByTagIDs(ctx, opts.TagIDs)
		if err != nil {
			logger.Errorf("[service] viewerWindowTotal failed: operation=imageRepo.CountByTagIDs error=%v", err)
		}
		return total, err
	}
	total, err := s.imageRepo.Count()
	if err != nil {
		logger.Errorf("[service] viewerWindowTotal failed: operation=imageRepo.Count error=%v", err)
	}
	return total, err
}
...
func (s *SearchService) viewerWindowImages(ctx context.Context, opts SearchOptions, offset, limit int) ([]domain.Image, error) {
	if opts.Query != "" {
		...
		if err != nil {
			logger.Errorf("[service] viewerWindowImages failed: operation=searchRepo.SearchImages error=%v", err)
		}
		return images, err
	}
	sortBy := opts.SortBy
	...
	if len(opts.TagIDs) > 0 {
		...
		if err != nil {
			logger.Errorf("[service] viewerWindowImages failed: operation=imageRepo.FindByTagIDs error=%v", err)
		}
		return images, err
	}
	images, err := s.imageRepo.FindAll(limit, offset, sortBy, sortOrder)
	if err != nil {
		logger.Errorf("[service] viewerWindowImages failed: operation=imageRepo.FindAll error=%v", err)
	}
	return images, err
}
