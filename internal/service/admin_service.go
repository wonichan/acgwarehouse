...
func (s *AdminService) GetSummary(ctx context.Context) (*Summary, error) {
	...
	if err != nil {
		logger.Errorf("[service] GetSummary failed: status=ready error=%v", err)
		summary.Tasks.Ready = 0
	}
	summary.Tasks.Running, err = s.jobRepo.CountByStatus("running")
	if err != nil {
		logger.Errorf("[service] GetSummary failed: status=running error=%v", err)
		summary.Tasks.Running = 0
	}
	summary.Tasks.Finished, err = s.jobRepo.CountByStatus("finished")
	if err != nil {
		logger.Errorf("[service] GetSummary failed: status=finished error=%v", err)
		summary.Tasks.Finished = 0
	}
	summary.Tasks.Failed, err = s.jobRepo.CountByStatus("failed")
	if err != nil {
		logger.Errorf("[service] GetSummary failed: status=failed error=%v", err)
...
func (s *AdminService) GetTaskPlatformOverview(ctx context.Context) (*TaskPlatformOverview, error) {
	...
	if err != nil {
		logger.Errorf("[service] GetTaskPlatformOverview failed: error=%v", err)
		return nil, err
	}
...
func (s *AdminService) ClearTaskQueue(ctx context.Context) (int, error) {
	logger.Infof("[service] ClearTaskQueue started")
	if s.taskRepo == nil || s.taskBatchRepo == nil {
		err := fmt.Errorf("task control repositories not configured")
		logger.Errorf("[service] ClearTaskQueue failed: error=%v", err)
		return 0, err
	}
	...
	logger.Infof("[service] ClearTaskQueue completed: cancelled_count=%d", count)
	return count, nil
}

func (s *AdminService) CancelTaskBatch(ctx context.Context, batchID int64) (int, error) {
	logger.Infof("[service] CancelTaskBatch started: batch_id=%d", batchID)
	if batchID <= 0 {
		err := fmt.Errorf("invalid batch_id")
		logger.Errorf("[service] CancelTaskBatch failed: error=%v", err)
		return 0, err
	}
	if s.taskRepo == nil || s.taskBatchRepo == nil {
		err := fmt.Errorf("task control repositories not configured")
	...
	logger.Infof("[service] CancelTaskBatch completed: cancelled_count=%d", count)
	return count, nil
}

func (s *AdminService) CancelTask(ctx context.Context, taskID int64) (int, error) {
	logger.Infof("[service] CancelTask started: task_id=%d", taskID)
	if taskID <= 0 {
		err := fmt.Errorf("invalid task_id")
		logger.Errorf("[service] CancelTask failed: error=%v", err)
		return 0, err
	}
	if s.taskRepo == nil || s.taskBatchRepo == nil {
		err := fmt.Errorf("task control repositories not configured")
	...
	logger.Infof("[service] CancelTask completed: cancelled_count=%d", count)
	return count, nil
}
...
func (s *AdminService) TriggerScan(ctx context.Context) (int64, error) {
	logger.Infof("[service] TriggerScan started")
	// The job manager will handle the scan via registered handler
	jobID, err := s.jobManager.AddJob(ctx, "manual_scan", "{}")
	if err != nil {
		logger.Errorf("[service] TriggerScan failed: error=%v", err)
		return 0, err
	}
	logger.Infof("[service] TriggerScan completed: job_id=%d", jobID)
	return jobID, nil
}
...
func (s *AdminService) RetryFailedJobs(ctx context.Context) (int, error) {
	logger.Infof("[service] RetryFailedJobs started")
	batches, err := s.listRetryableBatches(ctx)
	if err != nil {
		err := fmt.Errorf("task retry service not configured")
		logger.Errorf("[service] RetryFailedJobs failed: error=%v", err)
		return 0, err
	}
	...
	logger.Infof("[service] RetryFailedJobs completed: retried_count=%d", count)
	return count, nil
}

func (s *AdminService) RetryFailedBatchTasks(ctx context.Context, batchID int64) (*RetryBatchResult, error) {
	logger.Infof("[service] RetryFailedBatchTasks started: batch_id=%d", batchID)
	if batchID <= 0 {
		err := fmt.Errorf("invalid batch_id")
		logger.Errorf("[service] RetryFailedBatchTasks failed: error=%v", err)
		return nil, err
	}
	...
	logger.Infof("[service] RetryFailedBatchTasks completed: retried_count=%d", result.RetryCount)
	return result, nil
}

func (s *AdminService) RetryFailedTask(ctx context.Context, taskID int64) (*RetryBatchResult, error) {
	logger.Infof("[service] RetryFailedTask started: task_id=%d", taskID)
	if taskID <= 0 {
		err := fmt.Errorf("invalid task_id")
		logger.Errorf("[service] RetryFailedTask failed: error=%v", err)
		return nil, err
	}
	if s.taskBatchRepo == nil || s.taskRepo == nil {
		err := fmt.Errorf("task control repositories not configured")
	...
	logger.Infof("[service] RetryFailedTask completed: retried_count=%d", result.RetryCount)
	return result, nil
}
...
func (s *AdminService) PauseBackgroundTasks(ctx context.Context) error {
	logger.Infof("[service] PauseBackgroundTasks started")
	s.jobManager.Pause()
	return nil
}
...
func (s *AdminService) ResumeBackgroundTasks(ctx context.Context) error {
	logger.Infof("[service] ResumeBackgroundTasks started")
	s.jobManager.Resume()
	return nil
}
