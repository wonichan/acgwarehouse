...
func (s *COSService) Upload(ctx context.Context, filename, size string, data []byte) (string, error) {
	logger.Infof("[service] COS Upload started: filename=%s size=%s", filename, size)
	...
	if err != nil {
		logger.Errorf("[service] COS Upload failure: filename=%s size=%s error=%v", filename, size, err)
	...
	logger.Infof("[service] COS Upload completed: url=%s", finalURL)
	return finalURL, nil
}

func (s *COSService) DeleteByURL(ctx context.Context, objectURL string) error {
	logger.Infof("[service] COS DeleteByURL started: url=%s", objectURL)
	if s == nil || s.client == nil {
		return fmt.Errorf("cos service is not initialized")
	}

	if objectURL == "" {
		logger.Infof("[service] COS DeleteByURL skipped: empty url")
		return nil
	}

	u, err := url.Parse(objectURL)
	if err != nil {
		logger.Errorf("[service] COS DeleteByURL url parse failure: url=%s error=%v", objectURL, err)
		return fmt.Errorf("parse object url: %w", err)
	}

	key := strings.TrimPrefix(path.Clean(u.Path), "/")
	if key == "" || key == "." {
		logger.Infof("[service] COS DeleteByURL invalid key failure: url=%s key=%s", objectURL, key)
		return fmt.Errorf("invalid object key from url: %s", objectURL)
	}

	_, err = s.client.Object.Delete(ctx, key)
	if err != nil {
		logger.Errorf("[service] COS DeleteByURL deletion failure: url=%s key=%s error=%v", objectURL, key, err)
		return fmt.Errorf("delete object from cos: %w", err)
	}

	logger.Infof("[service] COS DeleteByURL completed: url=%s", objectURL)
	return nil
}
